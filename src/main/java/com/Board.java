...
public class Board extends JPanel implements Runnable {
        ...
        try {
            ...
            while (clock != null) {
                ...
                while (delta >= msPerFrame) {
                    ...
                    update();
                    // repaint component
                    repaint(ball.getX() - ball.getWidth(), ball.getY() - ball.getHeight(), ball.getWidth() * 2,
                            ball.getHeight() * 2);
                    repaint(0, paddle.getY(), Commons.SCREEN_WIDTH, paddle.getHeight() * 2);
                    repaint(item.getX() - item.getWidth(), item.getY() - item.getHeight(), item.getWidth() * 2,
                            item.getHeight() * 2);
    ...
    private void update() {
        ...
        if (item.getY() > paddle.getY()
                && (item.getX() >= paddle.getX() && item.getX() <= paddle.getX() + paddle.getWidth())) {
            touchItem(item);
            item = new Item(100, 150, 998);
        }
